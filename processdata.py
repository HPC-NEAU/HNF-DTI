...
def get_groups(train_ids, val_ids):
    ...
    val_len= len(val_ids)
    val_groups = np.zeros(val_len)
    train_foldlen = 1460
    train_groups = np.ones(train_len+10)
    train_groups[train_foldlen:2 * train_foldlen] *= 2
    train_groups[2 * train_foldlen:3 * train_foldlen] *= 3
    train_groups[3 * train_foldlen:] *= 4    

    groups=np.append(val_groups,train_groups)

    return groups


def load_data(data_root, dataset='cross_reverse', start_epoch=0, end_epoch=2000, common_neibor=3, neg_common_neibor=1, adj_norm=True, crossval=0):
    """
    load data
    ...
    if crossval:
        ...
        #groups = get_groups(train_ids, val_ids)
